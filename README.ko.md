...
- **`/claudex-status`** — Claude 실제 컨텍스트 사용량 + 남은 토큰, 그리고 Codex 상태(로그인·버전·모델·MCP 연결)를 표시.
...
간결한 상태 블록 — **Claude 컨텍스트**: 실제 사용량, 컨텍스트 창(자동감지 200k/1M), **남은 토큰**(현재 적응형 단계 포함); **Codex**: 로그인·버전·모델·MCP 연결. 사용량은 트랜스크립트의 실제 API 토큰 수에서 읽습니다. 요금제/청구 사용량은 내장 `/cost` 를 쓰세요.
...
사용량은 트랜스크립트의 **실제 API 토큰 수**(input + cache 읽기/생성)에서 읽고, 컨텍스트 창은 **자동 감지**(200k, 사용량이 200k를 넘으면 1M)합니다. 조정:

| 환경변수 | 기본값 | 의미 |
|---|---|---|
| `CODEX_CONTEXT_BUDGET` | _(자동)_ | 컨텍스트 창 수동 지정(미설정 시 200k/1M 자동감지) |
