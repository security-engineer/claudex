...
이 저장소는 플러그인 하나(**`claudex`**)를 담은 **Claude Code 플러그인 마켓플레이스**입니다.

## 플러그인이 설치해 주는 것

- **`codex` MCP 서버** — 번들된 `.mcp.json`으로 자동 등록 (`codex mcp-server` 실행).
- **`codex` 스킬** — 균형 위임 워크플로우 + 안전 수칙 (필요할 때만 로드되어, 쓰기 전엔 컨텍스트 비용 0).
- **`/claudex:delegate <작업>`** — 집중된 작업 하나를 Codex에 위임한 뒤 검토.
- **`/claudex:cross-review <대상>`** — 사용자가 라운드 수를 정하는, 경계가 있는 Claude↔Codex 피드백 루프.
- **`/claudex:mode`** — Claude↔Codex 작업 분할 방식 선택 (`solo | balanced | codex | max`).
- **`/claudex:status`** — 현재 작업 분할 모드와 Codex 상태(로그인·버전·모델·MCP 연결)를 표시.
...
# 세션 안에서:  /mcp   →   /claudex:delegate hello   →   /claudex:cross-review <대상>
```
...
**예시**
- `/claudex:delegate src/auth/ 의 diff를 리뷰하고 엣지케이스를 나열해줘`
- `/claudex:delegate utils/date.py 의 중복 제거 리팩터 초안을 작성해줘`
- `/claudex:delegate tests/test_api.py 의 실패 테스트 근본 원인을 찾아줘`

Codex는 기본적으로 **읽기 전용(read-only)** 으로 동작하며, 패치가 명시적으로 필요할 때만 씁니다. 비밀값이나 거대한 데이터셋 폴더는 범위에 넣지 마세요.

### `/claudex:cross-review <대상> [rounds=N] [style=ab|independent|debate]`
...
**예시**
- `/claudex:cross-review src/auth.py` — 2라운드, A→B.
- `/claudex:cross-review the staged diff rounds=4` — 최대 4라운드.
- `/claudex:cross-review "이 작업에 큐 vs 크론?" style=independent` — 독립 후 비교.
- `/claudex:cross-review payments/refund.py rounds=3 style=debate` — 3라운드 적대적 토론.
...
Claude↔Codex 작업 분할 방식을 선택합니다 (`~/.claude/claudex-mode`에 저장, 매 턴 읽음 — 다음 턴부터 적용). 인자 없으면 현재 모드 표시.

| 모드 | 작업 분할 |
|---|---|
| `solo` | Claude가 전부; Codex는 명시적으로 요청할 때만. |
| `balanced` (기본) | 탐색·초안·제2의견 리뷰 → Codex; Claude는 범위지정·검토·적용·테스트·판정. |
| `codex` | 드래프팅·다중파일 읽기·대부분 생성 → Codex; Claude는 조율·판정. |
| `max` | 거의 전부 → Codex; Claude는 조율·판정만. |

### `/claudex:status`

간결한 상태 블록 — 현재 **작업 분할 모드**, 그리고 **Codex**: 로그인·버전·모델·MCP 연결. Claude 자체 사용량은 내장 `/usage`(요금제 쿼터)·`/context`(컨텍스트 창)를 쓰세요 — 이 플러그인은 그걸 재발명하지 않습니다.

### `codex` 스킬

**필요할 때만** 로드됩니다(쓰기 전엔 컨텍스트 비용 0). 작업이 탐색·초안·엣지케이스 헌팅에 무겁거나 독립적인 제2 의견이 필요할 때 — 또는 Codex/토큰 절약 위임을 언급할 때 — Claude가 자동으로 호출합니다. 균형 자세, 위임 프롬프트 템플릿, 안전 수칙이 담겨 있습니다. 명시적으로 부르려면: "codex 워크플로우를 써줘."
...
- **`codex-reply`** — 스레드 이어가기: `threadId` + `prompt`. 다회차 `/claudex:cross-review`의 동력.
...
| 증상 | 해결 |
|---|---|
| `/claudex:delegate`, `/claudex:cross-review`, codex MCP 도구가 안 보임 | Claude Code 재시작 — MCP·명령은 세션 시작 시 로드됨 |
...
```bash
# plugins/claudex/.claude-plugin/plugin.json 의 "version" 을 올린 뒤:
git add -A && git commit -m "describe the change" && git push
```

사용자는 `claude plugin update claudex@claudex`로 받아갑니다(적용하려면 재시작).
